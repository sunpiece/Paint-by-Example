    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                ref_p = Image.open(
                    opt.reference_path).convert("RGB").resize((224, 224))
                ...
                if not opt.skip_save:
                    for i, x_sample in enumerate(x_checked_image_torch):
                        ...
                        img.save(os.path.join(
                            grid_path,
                            'grid-' + filename[:-4] + '_' + str(opt.seed) + '.png'))

                        x_sample = 255. * rearrange(x_sample.cpu().numpy(), 'c h w -> h w c')
                        img = Image.fromarray(x_sample.astype(np.uint8))
                        img = put_watermark(img, wm_encoder)
                        img.save(os.path.join(
                            result_path, filename[:-4] + '_' + str(opt.seed) + ".png"))
                        ...
                        inpaint_img.save(os.path.join(
                            sample_path, filename[:-4] + '_' + str(opt.seed) + "_inpaint.png"))
                        ...
                        ref_img.save(os.path.join(
                            sample_path, filename[:-4] + '_' + str(opt.seed) + "_ref.png"))
