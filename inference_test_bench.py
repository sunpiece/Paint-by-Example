...
# from imwatermark import WatermarkEncoderbabash
        ...
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                for test_batch, test_model_kwargs, segment_id_batch in test_dataloader:
                    test_model_kwargs = {
                        n : test_model_kwargs[n].to(
                            device, non_blocking=True) for n in test_model_kwargs
                    }
                    uc = None
                    if opt.scale != 1.0:
                        uc = model.learnable_vector.repeat(test_batch.shape[0], 1, 1)
                    c = model.get_learned_conditioning(
                        test_model_kwargs['ref_imgs'].squeeze(1).to(torch.float16))
                    if c.shape[-1] == 1024:
                        c = model.proj_out(c)
                    if len(c.shape) == 2:
                        c = c.unsqueeze(1)
                    ...
                    test_model_kwargs['inpaint_mask'] = Resize(
                        [z_inpaint.shape[-1], z_inpaint.shape[-1]])(
                    ...
                    if not opt.skip_save:
                        for i, x_sample in enumerate(x_checked_image_torch):
                            ...
                            mask_save.save(
                                os.path.join(sample_path, segment_id_batch[i] + "_mask.png"))
                            ...
                            inpaint_img.save(
                                os.path.join(sample_path, segment_id_batch[i] + "_inpaint.png"))
                            ref_img = 255. * rearrange(all_img[2], 'c h w -> h w c').numpy()
                            ref_img = Image.fromarray(ref_img.astype(np.uint8))
                            ref_img.save(
                                os.path.join(sample_path, segment_id_batch[i] + "_ref.png"))
                            base_count += 1
